...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
    	...
    	try{
    	Bson filterKey = new Document("_id", new ObjectId(movieId));
    	Document result = this.moviesCollection.find(filterKey).first();
    	if(result!= null){
            return true;
    	}
    	return false;
    ...
    public Document getMovie(String movieId) {
        ...
        List<Variable<String>> let = Arrays.asList(new Variable<String>("id", "$_id"));
        List<Bson> lookupPipe = Arrays.asList(new Document("$match", new Document("$expr",new Document("$eq", Arrays.asList("$movie_id", "$$id")))), new Document("$sort", new Document("date", -1))); 
    ...
    public List<Document> getMoviesByCountry(String... country) {
        ...
        this.moviesCollection.find(queryFilter).projection(projection).into(movies);
    ...
    public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
        ...
        Bson sort = new Document("tomatoes.viewer.numReviews", -1);
        List<Document> movies = new ArrayList<>();
        moviesCollection
                .find(castFilter)
                .sort(sort)
